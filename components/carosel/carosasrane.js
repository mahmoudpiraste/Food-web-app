...
function caroasr () {
  ...
  setCookieresid('selected_res', selected_resturant_id, { path: '/' , HttpOnly:true});
    ...
    const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_APP_API_URL}/homepg/asranecaros`, fetcher)
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem("mywallet", data.wallet_charge),
        ...
        return (
            ...
            <div  className='caros'>
           <div className='title-container'><h2 className='title-caros'>عصرانه برسون</h2><div className="showall-sec"><a href="#"><button className='show-all'>نمایش همه</button></a></div></div>
           <Swiper
      modules={[Navigation]}
      spaceBetween={5}
      slidesPerView={1.15}
